...
/* Tne names are ordered according to below:

* SW_CNTRL3 SW_CNTRL2 SW_CNTRL1 Active Antenna
*    -         0         0      RF3_ANT_EXT1: EXT2 (J40)
*    -         0         1      RF3_ANT_EXT1: EXT1 (J10)
*    -         1         0      RF1_ANT_OB20: ANT20 (on-board)
*    -         1         1      RF1_ANT_OB30: ANT30 (on-board) */
char* strFrdmSheildAntDesc[]    = {"Ext2", "Ext1", "Ant20", "Ant30"};
#endif
...
#if defined(gBoard_Antenna_Diversity_Support_d) && (gBoard_Antenna_Diversity_Support_d > 0)
static void CT_FrdmShieldBoardAntSel(uint8_t antenna_selection);
#endif
...
#if defined(gBoard_Antenna_Diversity_Support_d) && (gBoard_Antenna_Diversity_Support_d > 0)
static uint8_t frdmShieldAnt = gDefaultFrdmShieldAnt_c;
#endif
...
#if defined(gBoard_Antenna_Diversity_Support_d) && (gBoard_Antenna_Diversity_Support_d > 0)
    gaConfigParams[gConfParamFrdmShieldAnt].paramType = gParamTypeMixed_c;
    FLib_MemCpy(gaConfigParams[gConfParamFrdmShieldAnt].paramName, "Diversity", 10);
    gaConfigParams[gConfParamFrdmShieldAnt].id = gDefaultFrdmShieldAnt_c;
    gaConfigParams[gConfParamFrdmShieldAnt].ptr = strFrdmSheildAntDesc;
#endif
...
#if defined(gBoard_Antenna_Diversity_Support_d) && (gBoard_Antenna_Diversity_Support_d > 0)
    (void)GpioOutputPinInit(frdmShieldPins, ARRAY_SIZE(frdmShieldPins));
#endif
...
#if defined(gBoard_Antenna_Diversity_Support_d) && (gBoard_Antenna_Diversity_Support_d > 0)
    case 'e':
        pConfig = &gaConfigParams[gConfParamFrdmShieldAnt];
        switch(pConfig->id)
        {   /* switch in order of Ant20, Ant30, Ext1, Ext2 */
            case 0: pConfig->id = 2; break;
            case 1: pConfig->id = 0; break;
            case 2: pConfig->id = 3; break;
            case 3: pConfig->id = 1; break;
            default:
                assert(FALSE);
        }
        /* apply the config immediately */
        frdmShieldAnt = pConfig->id;
        CT_FrdmShieldBoardAntSel(frdmShieldAnt);
        break;
#endif
...
static bool_t CT_ApplyPrintConfigParams(void)
{
    ...
    if(CT_IsShortcutMenuEnabled())
    {
        ...
        {
            frdmShieldAnt = gaConfigParams[gConfParamFrdmShieldAnt].id;
            CT_FrdmShieldBoardAntSel(frdmShieldAnt);
            bParamsUpdated = TRUE;
        }
#endif
...
#if defined(gBoard_Antenna_Diversity_Support_d) && (gBoard_Antenna_Diversity_Support_d > 0)
/*! *********************************************************************************
* \brief  FRDM Shield board X-FR-ANTDV-MPIFA or X-FR-ANTDV-IFA antenna selection
*
* SW_CNTRL3 SW_CNTRL2 SW_CNTRL1 Active Antenna
*    -         0         0      RF3_ANT_EXT1: EXT2 (J40)
*    -         0         1      RF3_ANT_EXT1: EXT1 (J10)
...
static void CT_FrdmShieldBoardAntSel(uint8_t antenna_selection)
{
    ...
    {
        if( (antenna_selection & (1<<index) ) == 0 )
        {
            GpioClearPinOutput(&frdmShieldPins[index]);
        }
        else
        {
            GpioSetPinOutput(&frdmShieldPins[index]);
        }
    }
}
