...
#if (cPWR_UsePowerDownMode)
static void PWR_AddWaitTime(uint32_t delay);
#endif
...
static uint64_t mLpmLastSleepDuration;
#endif

/* store planned ADV interval time for low power mode 9*/
#if defined(PWR_Mode9AdvOnLPTimerOnly) && (PWR_Mode9AdvOnLPTimerOnly == 1)
static uint32_t mPWR_AdvdeepSleepTimeMode9Ms  = 0U;
#endif
...
#endif
static uint8_t mLPMNoIdleIntr = 0U;
...
static void PWR_WaitFor32MhzReady(void)
{
    ...
    if ( count_down == 0u )
    {
        ...
        PWR_AddWaitTime(cPWR_WaitRFOscillatorSwitchOff);

        /* turn back the oscillator ON, it should recover now */
        RSIM->CONTROL = RSIM_CONTROL_MODIFY_FIELD(RSIM_CONTROL_BLE_RF_OSC_REQ_EN, 1u);
...
#if (cPWR_UsePowerDownMode)
/*---------------------------------------------------------------------------
 * Name: PWR_AddWaitTime
 * Description: - Generic Lowpower function to add a specif waiting time
 * Parameters:  - delay - The value of delay depends on the clock configuration.
 * Return: -
 *---------------------------------------------------------------------------*/
static void PWR_AddWaitTime(uint32_t delay)
{
    for (uint32_t i= delay; i>0U; i--)
    {
        PWR_NOP;
    }
}
...
#if defined(PWR_Mode9AdvOnLPTimerOnly) && (PWR_Mode9AdvOnLPTimerOnly == 1)
/*---------------------------------------------------------------------------
* Name: PWR_GetLastSleepDurationMS
* Description: -
* Parameters: -
* Return: -
*---------------------------------------------------------------------------*/
static uint32_t PWR_GetLastSleepDurationMS(void)
{
    uint32_t time;

    time = (uint32_t) (mLpmLastSleepDuration / 1000ULL);
...
#endif
    {
        ...
        if (abort == FALSE)
        {
            ...
            */
            ...
            lowpower_skipped = PWRLib_MCU_Enter_lls1_2_3();

#if cPWR_BLE_LL_Enable
            if ( lowpower_skipped == TRUE )
            {
                /* lowpower has been aborted, wait for Link layer to be in DSM first */
                BOARD_DBGLPIOSET(0,0);
                BOARD_DBGLPIOSET(0,1);
                BOARD_DBGLPIOSET(0,0);

                /*   Add time delay before turning ON the oscillator because we don t know
                when it was turned off previously (could happen immediatly after going to lowpower) */
                PWR_AddWaitTime(cPWR_WaitLlDelayOnLateIrq);
                BOARD_DBGLPIOSET(0,1);
            }
            ...
            {
                /* This is safe to do a SW request to Link layer layer */
                PWR_DBG_LOG("SW request");
                BOARD_DBGLPIOSET(5,0);
                RSIM->CONTROL = RSIM_CONTROL_MODIFY_FIELD(RSIM_CONTROL_RSIM_DSM_EXIT, 1U);
                BOARD_DBGLPIOSET(5,1);
            }

#else   // cPWR_BLE_LL_Enable
            RSIM->CONTROL = RSIM_CONTROL_MODIFY_FIELD(RSIM_CONTROL_RF_OSC_EN, rfOscEn);
            (void)lowpower_skipped;
#endif
...
static void PWR_HandleDeepSleepMode_6(void)
{
    ...
    uint32_t lptmrFreq;
#endif /* (gTMR_EnableLowPowerTimers_d) */
    /* prevent stop mode if next LL activity has no idle interrupt */
    if (mLPMNoIdleIntr == 0U)
    {
        BOARD_DBGLPIOSET(3,0);
    ...
    if(enterLowPower)
    {
        ...
        if (abort == FALSE)
        {
            ...
            * and save the spend time in ticks that wasn't counted.        */
            ...
            {
                /* give some time for LL to be fully in DSM mode, otherwise, it will ignore the SW wakeup request */
                PWR_AddWaitTime(cPWR_WaitLlDelayOnLateIrq);
            }
        ...
        if (abort == FALSE)
        {
            ...
            * and save the spend time in ticks that wasn't counted.        */
...
#if defined(PWR_Mode9AdvOnLPTimerOnly) && (PWR_Mode9AdvOnLPTimerOnly == 1)
            /* Restart ADV on lowpower timer expiration only */

            /* Normal wakeup if wakeup source is not LPTMR  */
            if ( PWRLib_MCU_WakeupReason.Bits.FromLPTMR != 1u )
            {
                ...
                /* set low power time to trigger next planned ADV at the expected moement  */
                /* need to take into account time already spent in low power since last ADV event */
                /* PWR_GetLastSleepDurationMS returns time spent on last low power sequence and is reset at each wakeup */
                mPWR_DeepSleepTimeMs -= PWR_GetLastSleepDurationMS();
            ...
            {
                /* Restart ADV if wakeup source is LPTMR */

                /* set deep sleep time for low power timer to ADV interval
                    as it could have been modified since wakeup from other sources*/
                mPWR_DeepSleepTimeMs = mPWR_AdvdeepSleepTimeMode9Ms;
...
static void PWR_SyncTimers(uint32_t notCountedTicksBeforeSleep, uint32_t lptmrFreq)
{
    ...
    mLpmLastSleepDuration = ((uint64_t)timerTicks) * 1000000U / lptmrFreq;
    mLpmTotalSleepDuration      += mLpmLastSleepDuration;
    ...
    *requireDsmEntry     = FALSE;
    ...
    if( RSIM_CONTROL_READ_FIELD(RSIM_CONTROL_BLE_RF_OSC_REQ_STAT) != 0U) /* If false it means that BLE_LL is already in DSM */
    {
        if( (BLE_LL_REG(CLOCK_CONFIG) & BLE_LL_CLOCK_CONFIG_LLH_IDLE_MASK) != 0U)
        {
            ...
            distanceToNextInstant = Controller_GetNextInstance(nextInstant, &mLPMNoIdleIntr);
...
#if defined(PWR_Mode9AdvOnLPTimerOnly) && (PWR_Mode9AdvOnLPTimerOnly == 1)
        mPWR_AdvdeepSleepTimeMode9Ms = deepSleepTimeMs;
#endif
    }
    else
    {
