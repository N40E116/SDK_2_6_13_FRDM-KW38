...
/* Queue used to store valid packets received on SerialManagerTask
   to be processed on FSCI_Task */
static anchor_t mReceivedValidPacketQueue;
#endif
...
void FSCI_commInit( const gFsciSerialConfig_t* pSerCfg )
{
    ...
    /* Prepare valid packets queue */
    MSG_InitQueue(&mReceivedValidPacketQueue);
...
static void FSCI_Task(osaTaskParam_t argument)
{
...
#endif
    {
        ...
        while(MSG_Pending(&mReceivedValidPacketQueue))
        {
            clientPacket_t *pFsciPacketToProcess = MSG_DeQueue(&mReceivedValidPacketQueue);
            /* pFsciPacketToProcess->structured.header.startMarker was reused for passing the fsci interface */
            assert(pFsciPacketToProcess->structured.header.startMarker != mFsciInvalidInterface_c);
            /* Process the client packet */
            (void)FSCI_ProcessRxPkt(pFsciPacketToProcess, (uint32_t)pFsciPacketToProcess->structured.header.startMarker);
        }
    }
}
                ...
                if( status == PACKET_IS_VALID )
                {
                    ...
                    #endif
                    {
                        ...
                        #endif
                        {
                            ...
                            {
                                #if defined(gFsciUseDedicatedTask_c) && (gFsciUseDedicatedTask_c == 1)
                                /* pFsciPacketToProcess->structured.header.startMarker is reused for passing the fsci interface */
                                pCommData->pPacketFromClient->structured.header.startMarker = mFsciSrcInterface;
                                /* Put received valid packet in the mReceivedValidPacketQueue queue */                                
                                (void)MSG_Queue(&mReceivedValidPacketQueue, pCommData->pPacketFromClient);
                                pCommData->pPacketFromClient = NULL;
...
void FSCI_transmitPayload( uint8_t OG, uint8_t OC, const uint8_t *pMsg, uint16_t msgLen, uint32_t fsciInterface )
{
    ...
    {
        /* Compute size */
        buffer_size = (uint16_t)(sizeof(clientPacketHdr_t) + msgLen + gFsci_TailBytes_c);
...
#if !defined(gFsciUseDedicatedTask_c) || (gFsciUseDedicatedTask_c == 0)
