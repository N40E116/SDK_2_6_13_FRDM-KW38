...
/* Flash memory limits */
#define mFlashStartAddress_c (0x00000000U)
#define mFlashEndAddress_c   ((uint32_t)FSL_FEATURE_FLASH_PFLASH_BLOCK_SIZE * (uint32_t)FSL_FEATURE_FLASH_PFLASH_BLOCK_COUNT)
...
#if defined(gFSCI_MemAllocTest_Enabled_d) && (gFSCI_MemAllocTest_Enabled_d)
bool_t FSCI_MemAllocTestCanAllocate(void *pCaller);
#endif
...
static gFsciErrorMsg_t mFsciErrorMsg = {
    {
        ...
        mFsciMsgError_c,
        (fsciLen_t)sizeof(clientPacketStatus_t),
    },
...
void FSCI_Error(uint8_t errorCode, uint32_t fsciInterface)
{
    ...
    if( mFsciErrorReported == 0u )
    {
        ...
        (void)Serial_SyncWrite( gFsciSerialInterfaces[fsciInterface], (uint8_t*)&mFsciErrorMsg, size );
...
bool_t FSCI_MsgModeSelectReqFunc(clientPacket_t * pData, uint32_t fsciInterface)
{
    ...
    pData->structured.header.len = (fsciLen_t)sizeof(uint8_t);
    return TRUE;
}
...
bool_t FSCI_MsgWriteExtendedAdrReqFunc(clientPacket_t * pData, uint32_t fsciInterface)
{
    ...
    pData->structured.header.len = (fsciLen_t)sizeof(clientPacketStatus_t);
    return TRUE;
}
...
{
    pData->structured.header.len = (fsciLen_t)sizeof(uint32_t);
#if (defined(FSL_FEATURE_SOC_SIM_COUNT) && (FSL_FEATURE_SOC_SIM_COUNT > 0U))
...
bool_t FSCI_MemAllocTest(clientPacket_t * pData, uint32_t fsciInterface)
{
    ...
    FLib_MemCpy(&mFSCI_MemAllocBufferTest[mFSCI_MemAllocBufferTestIndex], 
                pData->structured.payload, 9);
    ...
    if(mFSCI_MemAllocBufferTest[mFSCI_MemAllocBufferTestIndex].enableTimeOfNoAllocation == TRUE)
    {
        ...
        mFSCI_MemAllocBufferTest[mFSCI_MemAllocBufferTestIndex].endtimeOfFailureValue = systemTime +
          TmrMillisecondsToMicroseconds(mFSCI_MemAllocBufferTest[mFSCI_MemAllocBufferTestIndex].timeOfNoAllocationValue);
    }
...
*
* \return  TRUE        Criteria for a condition is met do not allocate buffer.
* \return  FALSE       Criteria for a condition is not met allocate buffer.
*
********************************************************************************** */
bool_t FSCI_MemAllocTestCanAllocate(void *pCaller)
{
    ...
    for( index = 0; index < gFSCI_NotAllocMemoryBufferSize_c; index++ )
    {
         if (mFSCI_MemAllocBufferTest[index].enable == TRUE)
         {
               ...
               ((uint32_t)pCaller >= mFSCI_MemAllocBufferTest[index].LRMinValue))
            {
                ...
                {
                    if(mFSCI_MemAllocBufferTest[index].numberOfBuffersValue > mFSCI_MemAllocBufferTest[index].counterNumberOfBuffers)
                    {
                        /* Return TRUE so buffer will no be allocated */
                        mFSCI_MemAllocBufferTest[index].counterNumberOfBuffers++;
                        return TRUE;
                    }
                    else if(mFSCI_MemAllocBufferTest[index].numberOfBuffersValue == mFSCI_MemAllocBufferTest[index].counterNumberOfBuffers)
                    {
                        mFSCI_MemAllocBufferTest[index].enable = FALSE;
                        continue;
                    }
...
bool_t FSCI_EnableBootloaderFunc(clientPacket_t * pData, uint32_t fsciInterface)
{
    ...
    if (FLib_MemCmpToVal((uint8_t const*)gBootImageFlagsAddress_c, 0xFF, (uint8_t)FSL_FEATURE_FLASH_PFLASH_BLOCK_WRITE_UNIT_SIZE) == TRUE)
    {
