                             ...
                             uint32_t linSourceClockFreq)
{
    ...
    else
    {
            ...
            kStatus_Success)
        {
            ...
            linCurrentState->framingError       = false;
...
lin_status_t LIN_LPUART_AutoBaudCapture(uint32_t instance)
{
    ...
    else
    {
        ...
        /* Check break time minimum. After autobaud calculation the break detect still needs to be left open,
           in case the break field is sent by master before the current frame completes. */
        if (tmpTime >= AUTOBAUD_BREAK_TIME_MIN)
        {
            /* If the break field is dected as framing error, clear the flag so that the framing error enent
               will not be invoked */
            if (linCurrentState->framingError == true)
            {
                linCurrentState->framingError = false;
            }
            /* If the current node state is still receive/send data when break detected, this indicate the lst frame's
                ...
                (linCurrentState->currentNodeState == LIN_NODE_STATE_SEND_DATA))
            {
                ...
                {
                    /* Change the node's current event to NO RESPONSE */
                    linCurrentState->currentEventId = LIN_NO_RESPONSE_ERROR;
                }
                else
                {
                    /* Change the node's current event to LAST RESPONSE TOO SHORT */
    ...
    else
    {
        ...
        else
        {
            ...
            if (0U != (LIN_LPUART_GetStatusFlags(base) & (uint32_t)kLPUART_FramingErrorFlag))
            {
                ...
                /* Set frming error to true, do not abort current frame right now because premature break field
                   (13 bits of zero) can also cause framing error */
                linCurrentState->framingError = true;
            }
            else
            {
